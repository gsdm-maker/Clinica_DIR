...
BEGIN
    ...
    WITH movements_summary AS (
        SELECT
            m.producto_id,
            LOWER(m.condicion) as condicion_normalizada,
    ...
    aggregated_stock_per_product AS (
        SELECT
            ...
            json_object_agg(
                ms.condicion_normalizada,
                ms.stock_change
            ) FILTER (WHERE ms.condicion_normalizada IS NOT NULL AND ms.stock_change != 0) AS stock_breakdown_for_product
        FROM
